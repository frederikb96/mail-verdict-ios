...
#: the mutability, is what Swift 6 strict concurrency objects to.
...
def switch_after_guard_missing_return(lines: list[str]) -> list[tuple[int, str, str]]:
    ...
    is the shape every real instance of this bug has taken. A case whose body is indented
...
def unmarked_observer_token_read_in_deinit(
    ...
    the app target), so a Mac run is the first thing that does. The fix is
    `nonisolated(unsafe)`, safe when the property is written once on the main actor during setup
    and read once at deallocation, when nothing else holds a reference.

    Gated on `main_actor_line_numbers`: a plain, non-isolated class has no isolation boundary for
    its own `deinit` to cross, so the same read there is not an error — flagging it anyway is
    exactly the false positive that makes a check like this stop being trusted.
...
def stored_static_in_generic_type(lines: list[str]) -> list[tuple[int, str, str]]:
    ...
    in the app target, which compiles nowhere but a Mac).
