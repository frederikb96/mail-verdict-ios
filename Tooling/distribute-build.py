...
BUNDLE_ID = "com.frederikberg.mailverdict"
GROUP_NAME = "Internal"
...
def app_id() -> str:
    """The app's own App Store Connect id, looked up by bundle id.

    Not hardcoded: this script runs before the app record necessarily exists with a settled
    numeric id worth pinning, and a bundle id is the one identifier that cannot drift.
    """
    apps = call("GET", f"apps?filter[bundleId]={BUNDLE_ID}").get("data", [])
    if not apps:
        raise SystemExit(f"no App Store Connect app found for bundle id {BUNDLE_ID!r}")
    return apps[0]["id"]


def group_id(app: str) -> str:
    for group in call("GET", f"apps/{app}/betaGroups").get("data", []):
...
def main() -> None:
    ...
    for attempt in range(POLL_ATTEMPTS):
        builds = call("GET", f"builds?filter[app]={app}&sort=-uploadedDate&limit=10").get("data", [])
        if wanted:
