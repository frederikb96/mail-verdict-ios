import MailVerdictKit
    ...
    /// `/mailboxes/state` — collapsed sections and the scroll anchor, the two things Freddy's own
    /// verification ("same scroll position and collapse state") needs read back without a
    /// screenshot. `MailboxesStore` reports into `MailboxesDebugReporter` itself, since this
    /// route's handler is a synchronous, non-isolated closure that cannot hop to the main actor
