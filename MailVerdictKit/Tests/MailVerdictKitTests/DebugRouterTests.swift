    ...
    /// The bridge's value is that an agent — or a CI step — can trust what it returns. These
    /// cover the ways it could quietly stop being trustworthy: a request that is still arriving
    /// treated as malformed, a route that silently does not exist, and a body whose declared
    /// length lies.
    final class DebugRouterTests: XCTestCase {
        ...
        func testMethodIsPartOfTheRouteIdentity() {
            var router = DebugRouter()
            router.register("GET", "/mail") { _ in .message("read") }
            router.register("POST", "/mail") { _ in .message("write") }

            let read = String(decoding: router.handle(.init(method: "GET", path: "/mail")).json, as: UTF8.self)
            let write = String(decoding: router.handle(.init(method: "POST", path: "/mail")).json, as: UTF8.self)
        ...
        func testEncodedResponseLeavesSlashesAlone() {
            let response = DebugRouter.Response.encoding(["path": "/var/log/x"])
            XCTAssertTrue(String(decoding: response.json, as: UTF8.self).contains("/var/log/x"))
        }
    }
