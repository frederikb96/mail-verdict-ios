...
/// `text/event-stream`, a blank line is the record terminator; `MVSseEventAccumulator` frames on
/// exactly that, so a line source that drops it makes every record un-terminated and no event is
/// ever decoded.
///
/// Handles `\n`, `\r\n`, and a lone `\r` as line endings. A `\r` at the very end of a chunk is
/// held back rather than treated as a terminator, since the next chunk may complete it into
/// `\r\n`; the network can split a chunk anywhere, including inside a two-byte terminator.
struct MVLineSplitter: Sendable, Equatable {
    private var buffer: [UInt8] = []

    /// Feeds one chunk of raw bytes, in whatever boundaries the network happened to deliver them
    /// (see `MVHttpByteStream`), and returns every complete line the chunk finishes — zero, one,
