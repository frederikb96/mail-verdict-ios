...
// they hold the logic worth testing.
    ...
    ///
    /// **Why this reaches the host at all:** a simulator shares the build host's network stack,
    /// so a listener bound to 127.0.0.1 inside the app is reachable as `127.0.0.1` on the host —
    /// including from a GitHub Actions runner. No port forwarding, no `simctl` plumbing — `curl`
    /// just works. On a real device it would need a tunnel, which is one more reason the
    /// simulator is where this loop lives.
    ///
    ...
    public final class DebugBridge: @unchecked Sendable {
        ...
        private let queue = DispatchQueue(label: "mailverdict.debug-bridge")
