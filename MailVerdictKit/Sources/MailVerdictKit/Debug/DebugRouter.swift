    ...
    /// without a socket.
    ///
    /// The bridge exists so an agent — or a CI run — can read real application state instead of
    /// inferring it from pixels. A screenshot costs seconds and yields coordinates that break on
    /// any layout change; this costs milliseconds, yields JSON, and reaches state that is never
    /// on screen at all.
